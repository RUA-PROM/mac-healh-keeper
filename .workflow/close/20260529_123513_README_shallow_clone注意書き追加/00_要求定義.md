...
- 本 issue では **README 以外のドキュメント変更は対象外**（`docs/` 配下の仕様書同期は別 issue or 派生対応）。
- `scripts/lib/version_stamp.sh` の挙動変更（fallback 値変更等）は対象外。
- shallow clone 自体を install.sh で検出し警告を出す仕組みは別 issue 候補。

---

## 6. 成功基準（必須: 1 件以上）

- **基準 1**: README.md（リポジトリ直下）に shallow clone 注意書きが追記されている。
- **基準 2**: 実機で `git clone --depth=1 <repo> /tmp/test` → `cd /tmp/test` → `./install.sh` を実行した結果、`CFBundleVersion` が `0.0.0-DEV` または同等の fallback 値であることを再現確認できる。
- **基準 3**: 実機で `git fetch --tags --unshallow` 後の `./install.sh` で `CFBundleVersion` が `git describe --tags --always` の出力と一致することを確認できる。
- **基準 4**: `make check` が緑のまま維持される。
- **基準 5 (実機検証)**: ローカル環境で `git clone --depth=1` → `./install.sh` → CFBundleVersion が `0.0.0-DEV` fallback になることを実機で再現確認し、続けて `git fetch --tags --unshallow` 後の install で正しい stamp 値になることを実機で確認したエビデンス（コマンド出力・確認手順・結果）が `04_review.md` または `memo/` 配下に記録されていること。本基準の根拠は [`.agents-project/受け入れ基準ルール.md`](../../.agents-project/受け入れ基準ルール.md) §3.2–3.4。
