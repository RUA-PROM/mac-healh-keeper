...
### 2.3 (追補) 新規モジュール構成（2026-05-29）

ユーザー指示「新規機能追加」を受け、関数化ではなく **独立したライブラリ・コマンド** として切り出す。

| 新規ファイル | 種別 | 責務 |
|---|---|---|
| `scripts/lib/launchagent_lifecycle.sh` | bash library | `load_launchagent <label> <plist>` 関数を提供。bootout（idempotent）→ bootstrap → verify（print 経由）。stderr は構造化ログとして stdout に出す。`LAUNCHCTL_BIN` 環境変数で launchctl 実体を上書き可能（テスト用）。 |
| `scripts/lib/plist_validator.sh` | bash library | `validate_plist <file>` 関数。`plutil -lint` + 必須キー（Label, ProgramArguments）の grep ベース確認。 |
| `scripts/bin/launchagent-doctor.sh` | bash executable | 4 件 LaunchAgent の状態を `launchctl print` で診断し、人間可読サマリ + 1 行ごとの構造化ログを出力。exit code は失敗件数（最大 1）。 |
| `Sources/MacHealthKit/LaunchAgentStatus.swift` | Swift pure type | `LaunchAgentStatus { label, status: Status (.running/.notRunning/.notFound/.unknown), printOutputExcerpt }` と `LaunchAgentStatus.parse(label:printOutput:)` 静的メソッド。Functional Core。 |

### 2.4 (追補) 検証 API の置換

- 旧: `launchctl list \| grep <label>` — RunAtLoad=false な docker plist で偽陽性が発生（`memo/20260529_204726_root-cause-investigation.md` §2）
- 新: `launchctl print gui/<uid>/<label>` — exit 0 かつ出力に "Could not find service" を含まなければ loaded と判定。`LaunchAgentStatus.parse` がこの判定を pure 関数として実装する。
