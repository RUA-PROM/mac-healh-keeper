...
`create-release.yaml` を `macos-latest` に変更し、`make build` を呼んで `.app` 組み立て + stamp を行う。`.app` を ZIP 化（`ditto`）し、Release artifact として upload。tag 命名は **既存日時 tag 路線を維持**（破壊的変更を避ける）。
