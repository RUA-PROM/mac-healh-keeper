...
> **2026-05-29 実装フェーズ追補**: 実装方針を「create-release.yaml を最小差分で書き換える」から「**新規 `release-app.yml` を追加 + 既存 `create-release.yaml` には dispatch ステップを 1 つだけ追加**」に更新した（新規機能追加 posture）。新規スクリプト `scripts/lib/release_artifact_validator.sh` + 専用テスト `scripts/test/release_artifact_validator_test.sh` / `workflow_release_app_test.sh` / `ci_release_smoke_test.sh` を追加し、徹底テストで担保する。**Issue B（Makefile build .app 化, PR #8）は完了済**。

各タスクの最後に **必ず実機 `make build` → `make package` → artifact validator + Info.plist 検証**を含める。
