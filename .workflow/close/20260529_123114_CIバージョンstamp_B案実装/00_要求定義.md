...
> - 関連 issue: [`.workflow/20260529_122727_Makefile_app化拡張/`](../../20260529_122727_Makefile_app化拡張/) — B 案実装の前提として `make build` で `.app` 化が必要。
> - 本 issue ではドキュメント（00〜03）のみ作成し、実装着手は本依頼の対象外。
...
- **runner 変更**: `create-release.yaml` の runner を `ubuntu-latest` から **`macos-latest`** に変更（`plutil` 必須のため）。
- **checkout の `fetch-depth: 0`**: `git describe` で tag 履歴を取得するため、`actions/checkout@v4` の `fetch-depth: 0` を必須化。
- **.app 組み立て**: `make build`（または `make build-app` / Issue B の成果物）を CI で実行し `build/MacHealth.app` を生成。
- **CFBundleVersion 注入**: 採用方式は 02 で議論するが、原則は次の 2 択。
  - 方式 a: semver tag 値（例: `v1.4.0`）を CFBundleVersion に直接注入
  - 方式 b: `git describe --tags --always` 値を注入（既存 `version_stamp.sh` 流用）
- **artifact upload**: `actions/upload-release-asset@v1` 等で `.app.zip` を Releases に upload。
- **A 案との共存**: install.sh の stamp 動作は変更しない。
