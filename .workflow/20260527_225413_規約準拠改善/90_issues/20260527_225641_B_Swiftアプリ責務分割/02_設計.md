...
| `ShellRunner`（protocol＋既定実装 `ZshShellRunner`） | Infra | `src/` | 外部コマンドを実行し標準出力文字列を返す。実行ファイル＋引数配列の I/F を持つ。 |
| `JobController` | Infra | `src/` | launchctl による command（load/unload）と query（list 解析）を**別メソッド**で提供する（CQRS の Infra 部）。 |
...
- **`src/`（`swiftc` ビルド・AppKit/launchctl 依存）**
  ...
  - `ShellRunner.swift` … `protocol ShellRunner` ＋ 既定実装 `ZshShellRunner`。
  - `MetricsCollector.swift` … `MetricsCollector`（Imperative Shell）。
  - `JobController.swift` … `JobController`（CQRS の Infra 部）。
  - `MenuBuilder.swift` … `MenuBuilder`（`MenuItemSpec` 配列 → `NSMenu` 変換）と `NSMenuItem.toptip` 拡張。

> 配置は **AppKit 依存の有無**で決める。AppKit を import しないものだけ `MacHealthKit` に置く（=テスト可能）。`MenuModel`/`MenuItemSpec` は `NSMenu` を作らず**データのみ**を生成するため AppKit 非依存にでき、`MacHealthKit` に置ける（UC1-S1 を XCTest で固定するための要）。
...
- **install.sh**: `src/` に追加する `ShellRunner.swift`・`MetricsCollector.swift`・`JobController.swift`・`MenuBuilder.swift` と、`Sources/MacHealthKit/` の `Metrics.swift`・`MetricsParser.swift`・`JobCatalog.swift`・`MenuModel.swift`・既存 `ScheduleTiming.swift` を `swiftc` のビルド行（現 install.sh L80）に追加する。出力名 `MacHealth`・.app バンドル化・LaunchAgent・Info.plist 配置は不変。
- **SwiftPM**: `MacHealthKit` library に追加ファイル（`Metrics.swift`/`MetricsParser.swift`/`JobCatalog.swift`/`MenuModel.swift`）が含まれる（`path: "Sources/MacHealthKit"` のため自動）。`src/` の AppKit 依存ファイルは Package には含めない（現状方針維持）。テストは `Tests/MacHealthKitTests/` に追加する。
